...
# 1. .Type could tell you what type of car you have(luxury, sedan, SUV, etc)
# 2. .size could tell you how if the car is thought of as a big, medium, or small car
# 3. .speed will tell you the speed of the car, either slow, trucking, or lighting speed

# One method of your class, other than __init__, will be named...
# Think: anything you want a car to do. 
#  Change license registration and print something out? Drive forward? 
car_class_method = "Drivers Info"
...
car_method_description = "This will display all of the information of the cars Driver like name, gender, age, weight"
...
###
flickr_key = "08e1bae2c6ffc9326429a04d949947c5" # paste your flickr key here
...
#a = flickrREST(extra_params = {'tags':'mountains', 'per_page':10}).url
...
#	 have that tag.
team = flickrREST(extra_params = {'tags': tag, 'per_page': 50})
swag = fix_flickr_resp(team.text)
fin = json.loads(swag)
#print pretty(fin)
# 3) Extract each of the photo ids from that response, making a list of all of them.
m4x = []
for a in fin['photos']['photo']:
    m4x.append(a['id'])
#print m4x
...
fixthis = {}
for c in m4x:
    ay = flickrREST(method = 'flickr.photos.getInfo', extra_params = {'photo_id': c})
...
tags = []
for f in fixthis:
    taginfo = fixthis[f]['photo']['tags']['tag']
    for h in taginfo:
...
almost = sorted(new_freq, key = lambda x: new_freq[x], reverse = True)
done = almost[1:6]
print done
