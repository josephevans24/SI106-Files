...
class TextInformation:
    """A class used for helping the user discover simple information about large text files"""
    ...
    def word_count(self, name):
        lower_case = self.txt.lower()
        txt = lower_case.replace("?", "").replace('"', "").replace(".", "").replace("!", "").replace(",", "")
        new_name = name.lower()
        count_it = txt.split()
        accum = len([word for word in count_it if word == new_name])
...
class ShannonGameGuesser:
    """A Shannon Game Guesser class that can tell you how predictive certain texts are based on another text file the user provides"""
    ...
    while first_choice != 1 and first_choice != 2:
        ...
        if first_choice == 2:
            ...
            if text_to_choose == "g":
                ...
                rules = spanish.read()
            if text_to_choose == "p":
                programming = open("Programming Textbook.txt", "r")
                rules = programming.read()
        if first_choice == 1:
            typeOfFile = ""
            secondTry = True
